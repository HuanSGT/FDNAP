...
struct network {
    ...
    void get_match(ll upper_bound = 0, ll im = 0) {

        //VI tmp_fa = {fa[hubs[0]], fa[hubs[1]], fa[hubs[2]]};
        //for (ll i = 0; i < del; ++i) fa[hubs[i]] = 0;

        match = upper_bound; way_match = 0;
        for (ll i = 1; i <= n; ++i) matched[i] = in_match[i] = false;

        for (ll i = 0; i < im; ++i) in_match[hubs[i]] = true;

        dfs_match(1,0);

        for (ll i = 0; i < im; ++i) in_match[hubs[i]] = false;
